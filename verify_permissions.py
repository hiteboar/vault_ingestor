...
async def test_permissions():
    ...
    # Simulate a user not in allowed_chat_ids and with no access yet
    state_store.is_user_allowed.return_value = False
    state_store.get_allowed_folders.return_value = []
    state_store.get_context.return_value = "default"
    
    # Test /list (should be ignored by _handle_message)
    update = mock_update(guest_id, "/list some_folder")
    await adapter._handle_message(update, MagicMock())
    update.effective_message.reply_text.assert_not_called()
    print("✓ Guest unknown message ignored")

    # Test /join (should be allowed even if not yet 'allowed')
    update = mock_update(guest_id, "/join CODE123")
    state_store.claim_invite.return_value = "invitado_folder"
    await adapter._handle_message(update, MagicMock())
    assert "Has sido invitado" in update.effective_message.reply_text.call_args[0][0]
    print("✓ Guest /join allowed for anyone")

    # Now simulate user with access
    state_store.is_user_allowed.return_value = True
    ...
    print("✓ Guest /list allowed folder works after joining")
    ...
    await adapter._handle_message(update, MagicMock())
    update.effective_message.reply_text.assert_not_called()
    print("✓ Guest blocked after revocation/expiration")

    print("\n--- Testing Admin Permissions ---")
    admin_id = 123
    state_store.is_user_allowed.return_value = False # Admin doesn't need this
    state_store.get_allowed_folders.return_value = []
    
    # Test /invite (allowed for admin)
