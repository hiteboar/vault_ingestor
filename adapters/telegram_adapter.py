...
class TelegramAdapter:
    ...
    async def _cmd_help(self, msg, is_admin: bool) -> bool:
        ...
        help_text = (
            ...
            "\\/update\\_check   → Comprobar y aplicar actualización de GitHub\n"
    ...
    async def _cmd_update_check(self, msg, is_admin: bool, target_tag: str = "") -> bool:
        ...
        try:
            ...
            if has_updates:
                ...
                msg_text += "🔄 Creando copia de seguridad y aplicando actualización..."
                await status_msg.edit_text(msg_text, parse_mode="Markdown")
                # Llama a apply_update (que hace la copia y actualiza, luego se reinicia solo)
                self.update_manager.apply_update(remote_tag)
                # El proceso terminará debido al self.restart() dentro de apply_update.
                return True
            elif target_tag and remote_tag == "not_found":
                msg_text += f"❌ *El tag `{target_tag}` no existe* o no está publicado como Release."
                await status_msg.edit_text(msg_text, parse_mode="Markdown")
            else:
                msg_text += "✅ *El sistema está en la versión más reciente solicitada.*"
                await status_msg.edit_text(msg_text, parse_mode="Markdown")
